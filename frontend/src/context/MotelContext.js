...
export const MotelProvider = ({ children }) => {
    const [currentMotelId, setCurrentMotelId] = useState(() => {
        return sessionStorage.getItem('motelId') || null;
    });

    const [availableMoteles, setAvailableMoteles] = useState(() => {
        const saved = sessionStorage.getItem('moteles');
        try {
    ...
    // Sincronizar desde sessionStorage en el montaje
    useEffect(() => {
        const syncFromStorage = () => {
            ...
            try {
                ...
                const savedId = sessionStorage.getItem('motelId');
                if (savedId) {
                    setCurrentMotelId(savedId);
                } else if (filtered.length > 0) {
                    const firstId = filtered[0].id || filtered[0].motelId;
                    setCurrentMotelId(firstId);
                    sessionStorage.setItem('motelId', firstId);
                }
            } catch (e) {}
        };

        window.addEventListener('motel-session-sync', syncFromStorage);
        syncFromStorage();
        return () => window.removeEventListener('motel-session-sync', syncFromStorage);
    }, []);

    // Persistir cambios de motelId
    useEffect(() => {
        if (currentMotelId) {
            sessionStorage.setItem('motelId', currentMotelId);
        }
    }, [currentMotelId]);
    ...
    useEffect(() => {
        if (availableMoteles && availableMoteles.length > 0) {
            sessionStorage.setItem('moteles', JSON.stringify(availableMoteles));
        }
    }, [availableMoteles]);

    const changeMotel = (id) => {
        setCurrentMotelId(id);
        sessionStorage.setItem('motelId', id);
